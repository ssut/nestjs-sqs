...
type ReceieveMessageResponse = ReceiveMessageResult;
type ReceiveMessageRequest = ReceiveMessageCommand;
...
function isConnectionError(err: Error): boolean {
  if (err instanceof SQSError) {
    return (err.httpStatusCode === 403 || err.code === 'CredentialsError' || err.code === 'UnknownEndpoint');
  }
  return false;
}
...
export class Consumer extends EventEmitter {
  ...
  public emit<T extends keyof Events>(event: T, ...args: Events[T]) {
    return super.emit(event, ...args);
  }

  public on<T extends keyof Events>(event: T, listener: (...args: Events[T]) => void): this {
    return super.on(event, listener);
  }

  public once<T extends keyof Events>(event: T, listener: (...args: Events[T]) => void): this {
  ...
  private async changeVisabilityTimeout(message: SQSMessage, timeout: number): Promise<ChangeMessageVisibilityCommandOutput> {
    try {
  ...
  private async changeVisabilityTimeoutBatch(messages: SQSMessage[], timeout: number): Promise<ChangeMessageVisibilityCommandOutput> {
    const cmd = new ChangeMessageVisibilityBatchCommand({
      QueueUrl: this.queueUrl,
      Entries: messages.map((message) => ({
        Id: message.MessageId,
        ReceiptHandle: message.ReceiptHandle,
